...
import type { Crypto, Trader, Transaction, ActivityItem } from '@/types';
...
export default function UserDashboard({ currentPage, onNavigate }: UserDashboardProps) {
  ...
  const [userTraders, setUserTraders] = useState<string[]>([]);
  ...
  const loadData = async () => {
    setIsLoading(true);
    try {
      const [cryptosData, tradersData, userTradersData, transactionsData, activitiesData, chartDataRaw] = await Promise.all([
        mockApi.getCryptos(),
        mockApi.getTraders(),
        user ? mockApi.getUserTraders(user.id) : Promise.resolve([]),
        user ? mockApi.getUserTransactions(user.id) : Promise.resolve([]),
        user ? mockApi.getUserActivity(user.id) : Promise.resolve([]),
        user ? mockApi.getUserChartData(user.id) : Promise.resolve([]),
      ]);

      setCryptos(cryptosData);
      setTraders(tradersData);
      setUserTraders(userTradersData.map(ut => ut.trader_id));
      setTransactions(transactionsData);
      setActivities(activitiesData);
      setChartData(chartDataRaw.map(d => ({ date: d.date, value: d.value })));
    } catch (error) {
  ...
  const handleFollowTrader = async (traderId: string) => {
    ...
    try {
      if (userTraders.includes(traderId)) {
        await mockApi.unfollowTrader(user.id, traderId);
        setUserTraders(prev => prev.filter(id => id !== traderId));
        showToast('Unfollowed trader', 'info');
      } else {
        await mockApi.followTrader(user.id, traderId);
        setUserTraders(prev => [...prev, traderId]);
  ...
  const handleCreateTransaction = async (type: 'deposit' | 'withdrawal', data: Partial<Transaction>) => {
    ...
    try {
      ...
      const newTransactions = await mockApi.getUserTransactions(user.id);

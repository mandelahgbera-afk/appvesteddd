...
import { getCryptos, getTraders, createTransaction, getUserFollowedTraders, followTrader, unfollowTrader } from '@/lib/supabase';
...
export default function UserDashboard({ currentPage, onNavigate }: UserDashboardProps) {
  ...
  const { activities, isLoading: actLoading, refetch: refetchActivity } = useActivity();
  const { holdings, chartData, isLoading: portLoading } = usePortfolio();
